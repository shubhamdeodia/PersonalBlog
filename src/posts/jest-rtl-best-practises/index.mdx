...
const button = <button type="button" data-testid="prtui-test-submit-button"> Submit </button>
```
...
expect(getByROle('alert')).toHaveTextContent(/alert/i)
```
...
Use jest's fakeTimers to mock date related functions or setTImeout, setInterval etc.
```
